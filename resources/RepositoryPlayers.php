...
class RepositoryPlayers extends Repository
{
    protected $table = null;
    protected string $name = 'players';
    protected string $scope = 'extas';
    protected string $pk = 'identity';
    protected string $itemClass = '\lifecraft\components\players\Player';
    protected string $repoSubject = 'players';
    ...
    {
        \lifecraft\components\UUid::generateFor($item);\lifecraft\components\encoders\EncoderSha1::encode($item, ['secret']);\lifecraft\components\repositories\IsExist::check($this, $item, ['identity']);
    ...
    protected function getRepoInstance()
    {
        if (!$this->table) {
            $this->table = new \extas\components\repositories\drivers\DriverFileJson([
                'table' => $this, 'path' => 'resources/', 'db' => 'system', 
            ]);
        }
